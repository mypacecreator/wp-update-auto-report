...
class WUAR_Release_Notes {
	...
	private function get_api_url( string $type, string $slug, string $version ): ?string {
		switch ( $type ) {
			...
			case 'Plugin':
				return sprintf(
					'https://api.wordpress.org/plugins/info/1.2/?action=plugin_information&slug=%s&fields=sections',
					urlencode( $slug )
				);

			case 'Theme':
				return sprintf(
					'https://api.wordpress.org/themes/info/1.1/?action=theme_information&slug=%s&fields=sections',
					urlencode( $slug )
				);
	...
	private function extract_notes( string $type, array $data, string $version ): ?string {
		if ( 'Core' === $type ) {
			// Core changelog はリスト形式
			if ( ! isset( $data[0]['content'] ) ) {
				return null;
			}
			return wp_strip_all_tags( $data[0]['content'] );
		}
